...
def capture_output(lib, iteration, release, env_name, tool) -> None:
    _path_to_logs_old = f"/media/nimashiri/DATA/testing_results/icse26/{tool}/{lib}/{iteration}/{release}/expr"
    if lib == 'torch':
        ...
        for pair in all_pairs:
            ...
            for api in current_files:
                ...
                if flag and re.findall(r'(\bbug\b|\bneq\b)',  api):
                    shell_command = ["/home/nimashiri/Benchmarking-DL-Fuzzers/execution/capture_log_deeprel.sh",lib, release, tool, env_name, api, str(iteration)]
                    subprocess.call(shell_command,shell=False)
                    
def detect_bugs(lib, iteration,_version, tool) -> None:
    _path_to_logs_old = f"/media/nimashiri/DATA/testing_results/icse26/{tool}/{lib}/{iteration}/{_version}/expr/{_version}.txt"
...
if __name__ == '__main__':
    ...
    for lib in ['torch']:
        ...
        for release in releases:
            ...
            # capture_output(lib, 1, release,env_name, 'DeepRel')
            detect_bugs(lib, 1, release, 'DeepRel')