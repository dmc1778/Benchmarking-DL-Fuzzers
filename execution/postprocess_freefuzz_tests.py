...
def capture_output(lib, iteration,_version, env_name, tool) -> None:
    _path_to_logs_old = f"/media/nimashiri/DATA/testing_results/icse26/{tool}/{lib}/{iteration}/{_version}"
    if lib == 'torch':
            ...
            for api_dir in current_apis:
                ...
                for file in list_apis:
                    ...
                    if api_dir in target_data:
                        shell_command = ["/home/nimashiri/Benchmarking-DL-Fuzzers/execution/capture_log_freefuzz.sh",lib, _version, tool, env_name, dir_, oracle, api_dir, str(iteration), file_to_run]
...
def detect_bugs(lib, iteration,_version, tool):
    ...
    _path_to_logs_old = f"/media/nimashiri/DATA/testing_results/icse26/{tool}/{lib}/{iteration}/{_version}/{_version}.txt"
            ...
            if "Processing file" in log[0]:
                try:
                    ...
                    if row['Buggy API'] == api_name:
                        ...
                        if match: #and row['Version'] == _version: row['Version']
                            output = [tool, lib, iteration, row['Commit'], _version, api_name, row['Log Rule'], log_merged]
...
def main():
    ...
    for lib in ['tf']:
        ...
        for release in releases:
            ...
            # capture_output(lib, 1, release,env_name, 'FreeFuzz')
