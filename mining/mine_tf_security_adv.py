...
def get_advisory_detail(link):
    sub_content = requests.get(link)
    page_soup2 = soup(sub_content.text, "html.parser")
    metaInfo = page_soup2.contents[3].contents[3].contents[1].contents[9].contents[1].contents[3].contents[1].contents[1].contents[3].contents[1].contents[1].contents[1].contents[1].contents[1].contents[1]
    affected_version_area = metaInfo.contents[3].contents[3]
    patched_version_area = metaInfo.contents[5].contents[3]
    return affected_version_area.contents, patched_version_area.contents
    ...
    for page_num in range(1, 43):
        ...
        for record in records.contents:
            if not isinstance(record, str):
                ...
                affected_version, patched_versions = get_advisory_detail(full_link)
                data_ = scrape_security_page(full_link)
                data_[0].update({'Title': record_title})
                data_[0].update({'Link': full_link})

                # data_list.append(data_)
                list_data = [full_link, data_[0]['Title'], data_[0]['Bug description'], str(affected_version), str(patched_versions)]
